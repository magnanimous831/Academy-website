// useBusTracking-mock.ts - Fixed version
...
// Mock data that matches the interfaces exactly
...
export function useBusTracking() {
    ...
    const fetchData = () => {
      try {
        ...
        setTimeout(() => {
          ...
          setStops(mockStops);
          setLoading(false);
          console.log(`✅ Mock data loaded: ${mockBuses.length} buses, ${mockRoutes.length} routes, ${mockStops.length} stops`);
        }, 1000);
      } catch (err) {
        console.error('Error fetching mock data:', err);
        setError('Failed to load mock bus data');
        setLoading(false);
      }
    };
  ...
  // Mock realtime updates (disabled for now)
  useEffect(() => {
    console.log('📡 Mock realtime updates setup (disabled)');
  }, []);
  ...
  const getEstimatedArrival = (busId: string, stopId: string): string | null => {
    ...
    // Mock estimation - returns random 5-15 minutes
  ...
  const refreshData = () => {
    setLoading(true);
    setTimeout(() => {
      // Randomly update some bus locations
      const updatedBuses = [...mockBuses].map(bus => {
        if (bus.status === 'active' && Math.random() > 0.5) {
          return {
            ...bus,
            current_lat: bus.current_lat ? bus.current_lat + (Math.random() - 0.5) * 0.01 : 40.7128,
            current_lng: bus.current_lng ? bus.current_lng + (Math.random() - 0.5) * 0.01 : -74.0060,
            last_location_update: new Date().toISOString()
          };
        }
        return bus;
